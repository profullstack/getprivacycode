...
`packages/privacycode/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/tui
name: @privacycode-ai/tui
```

Target dependency graph:

```text
packages/privacycode ---\
                      > @privacycode-ai/tui -> @privacycode-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/privacycode`,
`packages/cli`, or `@privacycode-ai/core`.
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/privacycode`.
- Do not replace private `packages/privacycode` imports with `@privacycode-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@privacycode-ai/tui`.

## Ownership Boundary

### `@privacycode-ai/tui` Owns
...
```text
packages/privacycode/src/cli/cmd/tui
```

Its private dependency on `packages/privacycode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/privacycode/src/*`.
...
Status: Completed. The private `@privacycode-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@privacycode-ai/tui`.
...
- The package has no dependency on `opencode`, `@privacycode-ai/cli`, or
  `@privacycode-ai/core`.
...
primitives now live in `@privacycode-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@privacycode-ai/tui/runtime`. Movable app, component,
...
`@privacycode-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/privacycode` initially.
...
canonical in `@privacycode-ai/tui`. Configured references resolve through the new
...
`@privacycode-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/privacycode`.
...
- The package has no imports from `packages/privacycode`, `packages/cli`, or
  `@privacycode-ai/core`.
...
public `@privacycode-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/privacycode`.
- Keep the legacy embedded worker and server startup in `packages/privacycode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@privacycode-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/privacycode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@privacycode-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@privacycode-ai/core`, or either
  executable package.
...
cd packages/privacycode && bun typecheck
...
rg '@privacycode-ai/core|packages/privacycode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/privacycode/src packages/privacycode/test
```
...
- Build the current-platform `packages/privacycode` binary.
