...
import { Tag } from "@privacycode-ai/ui/v2/badge-v2"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.privacycodeChecks[item.config.distro]
          const privacycodeAction = () => wslOpencodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-privacycode" && wsl.data.job.distro === item.config.distro
          return (
