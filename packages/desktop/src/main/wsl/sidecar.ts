...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export PRIVACYCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export PRIVACYCODE_CLIENT=desktop",
    `export PRIVACYCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export PRIVACYCODE_SERVER_PASSWORD=${shellEscape(password)}`,
