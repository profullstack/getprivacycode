...
// test/preload.ts pins PRIVACYCODE_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.PRIVACYCODE_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.PRIVACYCODE_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.PRIVACYCODE_MODELS_PATH = undefined
  Flag.PRIVACYCODE_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.PRIVACYCODE_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.PRIVACYCODE_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.PRIVACYCODE_DISABLE_MODELS_FETCH = false
        }),
