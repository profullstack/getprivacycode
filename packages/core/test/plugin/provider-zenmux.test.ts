...
import { ProviderPlugins } from "@privacycode-ai/core/plugin/provider"
import { ZenmuxPlugin } from "@privacycode-ai/core/plugin/provider/zenmux"
...
describe("ZenmuxPlugin", () => {
  ...
  it.effect("applies the exact legacy Zenmux headers", () =>
    Effect.gen(function* () {
      ...
      expect(result.request.headers).toEqual({ "HTTP-Referer": "https://privacycode.ai/", "X-Title": "privacycode" })
  ...
  it.effect("merges legacy Zenmux headers with existing headers", () =>
    Effect.gen(function* () {
      ...
      expect(required(yield* catalog.provider.get(ProviderV2.ID.make("zenmux"))).request.headers).toEqual({
        Existing: "value",
        "HTTP-Referer": "https://privacycode.ai/",
        "X-Title": "privacycode",
      })
    }),
  )
