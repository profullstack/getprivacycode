...
This is the living design and status document for `@privacycode-ai/codemode` and its existing V2 OpenCode adapter.
...
`packages/privacycode/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
