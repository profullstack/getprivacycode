...
- Running `bun dev` from `packages/privacycode` starts the live interactive TUI. Do not run it as a blocking foreground command when you need to inspect the result.
...
export class Service extends Context.Service<Service, Interface>()("@privacycode/Foo") {}
