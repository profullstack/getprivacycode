...
import type { StreamDetokenizer } from "@/privacy"
import { errorMessage } from "@/util/error"
import { isRecord } from "@/util/record"
import { EventV2Bridge } from "@/event-v2-bridge"
import { Database } from "@privacycode-ai/core/database/database"
import { Usage, type LLMEvent } from "@privacycode-ai/llm"
...
interface ProcessorContext extends Input {
  ...
  streamDetok: StreamDetokenizer | undefined
}

type StreamEvent = LLMEvent

export class Service extends Context.Service<Service, Interface>()("@privacycode/SessionProcessor") {}
    ...
    const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
      ...
      const handleEvent = Effect.fnUntraced(function* (value: StreamEvent) {
        switch (value.type) {
          ...
          case "text-start":
            ctx.streamDetok = PrivacyGuard.get().createStreamDetokenizer()
            ctx.currentText = {
              id: PartID.ascending(),
              messageID: ctx.assistantMessage.id,
              sessionID: ctx.assistantMessage.sessionID,
          ...
          case "text-delta":
            ...
            const detok = ctx.streamDetok?.push(value.text) ?? value.text
            ctx.currentText.text += detok
          ...
          case "text-end":
            if (!ctx.currentText) return
            const tail = ctx.streamDetok?.flush() ?? ""
            if (tail) ctx.currentText.text += tail
            // oxlint-disable-next-line no-self-assign -- reactivity trigger
