...
import type { LLMClientShape } from "@privacycode-ai/llm/route"
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @privacycode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      ...
      // the @privacycode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
