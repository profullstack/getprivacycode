...
import { InstallationLocal, InstallationVersion } from "@privacycode-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@privacycode-ai/core/v1/config/console-state"
import { FSUtil } from "@privacycode-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@privacycode-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@privacycode-ai/core/v1/config/config"
import { RemoteAuthError } from "@privacycode-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@privacycode-ai/core/v1/config/permission"
import { ConfigPluginV1 } from "@privacycode-ai/core/v1/config/plugin"
...
export class Service extends Context.Service<Service, Interface>()("@privacycode/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["privacycode.jsonc", "privacycode.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.PRIVACYCODE_CONFIG && !Flag.PRIVACYCODE_CONFIG_DIR && !Flag.PRIVACYCODE_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "privacycode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "privacycode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "PRIVACYCODE_CONFIG_CONTENT") return "local"
        ...
        if (Flag.PRIVACYCODE_CONFIG) {
          yield* merge(Flag.PRIVACYCODE_CONFIG, yield* loadFile(Flag.PRIVACYCODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.PRIVACYCODE_CONFIG })
        }
        ...
        if (Flag.PRIVACYCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from PRIVACYCODE_CONFIG_DIR", { path: Flag.PRIVACYCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".opencode") || dir.endsWith(".privacycode") || dir === Flag.PRIVACYCODE_CONFIG_DIR) {
            for (const file of ["privacycode.json", "privacycode.jsonc", "opencode.json", "opencode.jsonc"]) {
        ...
        if (process.env.PRIVACYCODE_CONFIG_CONTENT) {
          const source = "PRIVACYCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.PRIVACYCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from PRIVACYCODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["PRIVACYCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("PRIVACYCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.PRIVACYCODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.PRIVACYCODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("PRIVACYCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.PRIVACYCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.PRIVACYCODE_DISABLE_PRUNE) {
