...
import { isRecord } from "@privacycode-ai/tui/util/record"
...
import { TuiKeybind } from "@privacycode-ai/tui/config/keybind"
import { InstallationLocal, InstallationVersion } from "@privacycode-ai/core/installation/version"
import { makeRuntime } from "@privacycode-ai/core/effect/runtime"
import { Filesystem } from "@/util/filesystem"
import { ConfigVariable } from "@/config/variable"
import { Npm } from "@privacycode-ai/core/npm"
import { FormatError, FormatUnknownError } from "@/cli/error"
import { TuiConfig } from "@privacycode-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@privacycode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // folders between cwd and home, and PRIVACYCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.PRIVACYCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit PRIVACYCODE_TUI_CONFIG override, if set.
  if (Flag.PRIVACYCODE_TUI_CONFIG) {
    const configFile = Flag.PRIVACYCODE_TUI_CONFIG
  ...
  // 4. `.privacycode` and `.opencode` directories (and PRIVACYCODE_CONFIG_DIR)
  // discovered while walking up the tree.
  const dirs = unique(directories).filter(
    (dir) => dir.endsWith(".privacycode") || dir.endsWith(".opencode") || dir === Flag.PRIVACYCODE_CONFIG_DIR,
  )

  for (const dir of dirs) {
    if (!dir.endsWith(".privacycode") && !dir.endsWith(".opencode") && dir !== Flag.PRIVACYCODE_CONFIG_DIR) continue
