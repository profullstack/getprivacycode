...
export type PatchDeps = {
  ...
  files: (dir: string, name: typeof ConfigPaths.CONFIG_NAME | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ConfigPaths.PROJECT_DIR)
}

function patchName(kind: Kind): typeof ConfigPaths.CONFIG_NAME | "tui" {
  if (kind === "server") return ConfigPaths.CONFIG_NAME
  return "tui"
}
