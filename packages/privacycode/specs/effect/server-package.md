...
- The server still lives in `packages/privacycode`.
...
Until enough shared service code lives outside `packages/privacycode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/privacycode`

It should not import `packages/privacycode` services while `packages/privacycode`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/privacycode` implementation details.
