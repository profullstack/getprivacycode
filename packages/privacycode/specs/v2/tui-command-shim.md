...
Delete `packages/privacycode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/privacycode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/privacycode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/privacycode`
- TUI plugin loader tests in `packages/privacycode` if runtime plugin API wiring changed
