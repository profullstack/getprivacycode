...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.PRIVACYCODE_WEBSEARCH_PROVIDER

    try {
      process.env.PRIVACYCODE_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.PRIVACYCODE_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.PRIVACYCODE_WEBSEARCH_PROVIDER
      else process.env.PRIVACYCODE_WEBSEARCH_PROVIDER = original
    }
  })
