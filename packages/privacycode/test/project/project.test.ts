...
import { Hash } from "@privacycode-ai/core/util/hash"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const projectIdFile = path.join(tmp, ".git", "privacycode")
      expect(yield* Effect.promise(() => Bun.file(projectIdFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "privacycode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "privacycode")
      const wrongCache = path.join(parentDir, ".git", "privacycode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "privacycode")
      const cacheB = path.join(bareB, "privacycode")
      const wrongCache = path.join(parentDir, ".git", "privacycode")
