...
const cleanState = Effect.gen(function* () {
  ...
  delete process.env.PRIVACYCODE_CONFIG
  delete process.env.PRIVACYCODE_TUI_CONFIG
...
it.instance("project config takes precedence over PRIVACYCODE_TUI_CONFIG (matches PRIVACYCODE_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("PRIVACYCODE_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from PRIVACYCODE_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
