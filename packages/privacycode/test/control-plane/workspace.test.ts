...
import { EventSequenceTable } from "@privacycode-ai/core/event/sql"
...
import { WorkspaceTable } from "@privacycode-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  PRIVACYCODE_AUTH_CONTENT: process.env.PRIVACYCODE_AUTH_CONTENT,
  PRIVACYCODE_EXPERIMENTAL_WORKSPACES: process.env.PRIVACYCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.PRIVACYCODE_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.PRIVACYCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.PRIVACYCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.PRIVACYCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.PRIVACYCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
