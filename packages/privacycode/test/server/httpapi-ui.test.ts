  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("PRIVACYCODE_SERVER_PASSWORD", original.envPassword)
        restoreEnv("PRIVACYCODE_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            PRIVACYCODE_SERVER_PASSWORD: input?.password,
            PRIVACYCODE_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
